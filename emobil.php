...
$objPHPExcel->getProperties()->setCreator('MSN')
             ...
             ->setTitle("Data Mobil")
             ->setSubject("Mobil")
             ->setDescription("Laporan Semua Data Mobil")
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A1', "DATA MOBIL"); // Set kolom A1 dengan tulisan "DATA SISWA"
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('B3', "Kode Mobil"); // Set kolom B3 dengan tulisan "NIS"
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('C3', "Nomor Polisi"); // Set kolom C3 dengan tulisan "NAMA"
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('F3', "Model Mobil"); // Set kolom E3 dengan tulisan "TELEPON"
...
$objPHPExcel->setActiveSheetIndex(0)->setCellValue('I3', "Lokasi Mobil"); // Set kolom F3 dengan tulisan "ALAMAT"
...
$tampil=$mysqli->query("SELECT * FROM tbl_mobil  INNER JOIN brandm ON brandm.id_brand = tbl_mobil.id_brand INNER JOIN merk_b ON merk_b.id_tipe = tbl_mobil.id_tipe ORDER BY tbl_mobil.id_mobil");
...
while($data = mysqli_fetch_array($tampil)){ // Ambil semua data dari hasil eksekusi $sql
  ...
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('B'.$numrow, $data['id_mobil']);
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('C'.$numrow, $data['no_polisi']);
  ...
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F'.$numrow, $data['model_mobil']);
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G'.$numrow, number_format($data['harga'],2,',','.'));
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H'.$numrow, $data['tgl_masuk']);
  $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I'.$numrow, $data['lokasi_mobil']);
...
$objPHPExcel->getActiveSheet(0)->setTitle("Laporan Data Mobil");
